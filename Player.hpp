...
public:
    ...
    void fixedUpdate() override
    {
        if (isDead())
        {
            ...
            p_kinematic->rotation_velocity = 0;

            p_helper_kinematic->velocity_x = p_kinematic->velocity_x;
            p_helper_kinematic->velocity_y = p_kinematic->velocity_y;
            return;
        }
        // Look at the mouse
        ...
        {
            p_kinematic->velocity_x = -s_vel * (not p_sfml->input().isKeyDown(sf::Keyboard::D));
        }
        else if (p_sfml->input().isKeyDown(sf::Keyboard::D))
        {
            p_kinematic->velocity_x = s_vel;
        }
        else
        {
            p_kinematic->velocity_x = 0;
        ...
        {
            p_kinematic->velocity_y = -s_vel * (not p_sfml->input().isKeyDown(sf::Keyboard::S));
        }
        else if (p_sfml->input().isKeyDown(sf::Keyboard::S))
        {
            p_kinematic->velocity_y = s_vel;
        }
        else
        {
            p_kinematic->velocity_y = 0;
        ...
        if (p_kinematic->velocity_x != 0 or p_kinematic->velocity_y != 0)
        {
            p_sprite->play();
        }
        else
